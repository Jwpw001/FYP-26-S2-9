  ...
  return (
    <div style={{ background: "#FFFFFF", border: "1px solid #E2E8F0", borderRadius: "14px", overflow: "hidden", display: "flex", flexDirection: "column", flex: 1, minHeight: 0, height: "100%" }}>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #E2E8F0", background: "#F8FAFC", flexShrink: 0 }}>
        <div style={{ width: NAME_COL_WIDTH, flexShrink: 0, padding: "10px 14px", fontSize: "11px", fontWeight: "700", color: "#64748B", textTransform: "uppercase", letterSpacing: "0.04em", borderRight: "1px solid #E2E8F0" }}>
          Staff
        </div>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{ fontSize: "9px", fontWeight: "700", color: isToday ? "#2563EB" : "#94A3B8" }}>{DAY_LABELS[(d.getDay() + 6) % 7]}</div>
                <div style={{ fontSize: "11px", fontWeight: isToday ? "800" : "600", color: isToday ? "#2563EB" : "#1E293B" }}>{d.getDate()}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {rows.length === 0 ? (
          <div style={{ padding: "40px", textAlign: "center", fontSize: "13px", color: "#94A3B8" }}>{emptyLabel}</div>
        ) : (
          rows.map(row => {
            ...
            return (
              <div key={row.id} style={{ display: "flex", borderBottom: "1px solid #F1F5F9" }}>
                <div style={{ width: NAME_COL_WIDTH, flexShrink: 0, padding: "10px 14px", display: "flex", alignItems: "center", gap: "8px", borderRight: "1px solid #E2E8F0" }}>
                  <div style={{ width: "26px", height: "26px", borderRadius: "50%", background: row.avatarColor || "#6366F1", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: "700", flexShrink: 0 }}>
                    {row.name?.[0]?.toUpperCase() || "?"}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: "12px", fontWeight: "700", color: "#1E293B", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{row.name}</p>
                    {row.sublabel && <p style={{ fontSize: "10px", color: "#94A3B8" }}>{row.sublabel}</p>}
                  </div>
                </div>
                    ...
                    return (
                      <button key={bar.id} title={bar.title}
                        ...
                        style={{
                          ...
                          fontSize: "11px", fontWeight: "700", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
...
export function RangeNav({ rangeStart, numDays, onChange }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px", flexShrink: 0 }}>
      ...
      <span style={{ fontSize: "13px", fontWeight: "700", color: "#1E293B", minWidth: "180px", textAlign: "center" }}>
        {fmt(rangeStart)} – {fmtYear(rangeEnd)}
      </span>
      <button onClick={() => shift(numDays)} style={navBtnStyle}>→</button>
      <button onClick={() => onChange(startOfWeek(new Date()))} style={{ ...navBtnStyle, width: "auto", padding: "0 12px", fontSize: "12px", fontWeight: "600" }}>Today</button>
    </div>
  );
}
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 20000, background: "#FFFFFF", display: "flex", flexDirection: "column", padding: "20px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px", flexShrink: 0 }}>
        {title && <h2 style={{ fontSize: "18px", fontWeight: "800", color: "#1E293B" }}>{title}</h2>}
...
const navBtnStyle = {
  ...
  cursor: "pointer", fontSize: "14px", color: "#475569", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
};

...
const EMPTY_DRAFT = { title: "", skill_id: "", start_time: "", end_time: "", required_workers: 1 };
        ...
        <div style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: "14px", overflow: "hidden" }}>
          ...
          {dayRows.map((row, i) => (
            draft?.template_id === row.template_id ? (
              <TemplateDraftRow key={row.template_id} draft={draft} setDraft={setDraft} skills={skills} onSave={saveDraft} onCancel={() => setDraft(null)} saving={saving} />
            ) : (
          ...
          {draft && !draft.template_id && (
            <TemplateDraftRow draft={draft} setDraft={setDraft} skills={skills} onSave={saveDraft} onCancel={() => setDraft(null)} saving={saving} isNew />
          )}
        </div>
      )}
...
function TemplateDraftRow({ draft, setDraft, skills, onSave, onCancel, saving, isNew }) {

...
import { LayoutDashboard, Briefcase, CalendarClock } from "lucide-react";
import "./sidebarStyles.js";
import ProfileModal from "../ProfileModal";

const NAV = [
  { label: "Dashboard",    path: "/krewby-worker/dashboard",    Icon: LayoutDashboard },
  { label: "My Jobs",      path: "/krewby-worker/jobs",         Icon: Briefcase },
  { label: "Availability", path: "/krewby-worker/availability", Icon: CalendarClock },
];
...
export default function WorkerLayout({ children, title }) {
  ...
  const onNotifPage = location.pathname === "/krewby-worker/notifications";
    ...
    <div style={s.shell}>
      <aside
        ...
        <div style={{ ...s.sidebarTop, padding: "20px 14px 16px" }}>
          <Link to="/krewby-worker/dashboard" style={s.logoRow}>
          ...
          <div style={{ opacity: expanded ? 1 : 0, maxHeight: expanded ? "30px" : "0px", transition: "opacity 0.25s ease, max-height 0.25s ease", overflow: "hidden", marginTop: "8px" }}>
            <span style={s.workerBadge}>Krewby Worker</span>
          </div>
        </div>
        <nav style={s.nav}>
          {NAV.map(item => {
        ...
        <div style={{ ...s.sidebarBottom, padding: "12px" }}>
          <button onClick={() => setShowProfile(true)} title="View profile"
            ...
            <div style={{ ...s.avatar, flexShrink: 0 }}>{user?.full_name?.[0]?.toUpperCase() || "W"}</div>
            <div style={{ opacity: expanded ? 1 : 0, maxWidth: expanded ? "140px" : "0px", transition: "opacity 0.25s ease, max-width 0.25s ease", overflow: "hidden" }}>
              <p style={s.userName}>{user?.full_name || "Worker"}</p>
              <p style={s.userRole}>Krewby Casual Worker</p>
            </div>
          </button>
      ...
      <div style={s.main}>
        <header style={s.topbar}>
          ...
          <div style={{ flex: 1 }} />
          <button
            onClick={() => navigate("/krewby-worker/notifications")}
            style={{
...
const s = {
  ...
  workerBadge: {
    ...
    letterSpacing: "0.06em", textTransform: "uppercase", border: "1px solid rgba(59,130,246,0.3)",
    whiteSpace: "nowrap",
  },

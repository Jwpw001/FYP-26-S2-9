...
export default function WorkerJobs() {
  ...
  async function handleClockIn(assignmentId) {
    ...
    try {
      ...
      setJobs(prev => prev.map(j => j.assignment_id === assignmentId ? { ...j, clocked_in: true, clock_in: new Date().toISOString() } : j));
  ...
  async function handleClockOut(assignmentId) {
    ...
    try {
      ...
      setJobs(prev => prev.map(j => j.assignment_id === assignmentId ? { ...j, clocked_out: true, clock_out: new Date().toISOString() } : j));
  ...
  const filtered = jobs.filter(j => {
    if (filter === "upcoming") return j.shift_date >= today && j.status !== "declined";
    if (filter === "past") return j.shift_date < today;
    return true;
  });
        ...
        <div key={job.assignment_id} style={s.card}>
          <div style={s.cardTop}>
            <div>
              ...
              <p style={s.jobAddr}>{job.outlet_address}</p>
            </div>
          ...
          {job.status === "confirmed" && job.shift_date === today && (
            <div style={s.actions}>
              {!job.clocked_in ? (

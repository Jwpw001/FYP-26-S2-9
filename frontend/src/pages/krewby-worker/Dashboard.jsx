  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Service returns shift_assignments with nested: shifts, shift_roles, attendance
        const jobs = res.data || [];
        const today = new Date().toISOString().split("T")[0];
        const upcoming = jobs
          .filter(j => j.shifts?.shift_date >= today)
            ...
            <div style={s.section}>
              ...
              {upcomingJobs.map(job => (
                <div key={job.assignment_id} style={s.jobCard}>
                  <div>
                    {/* nested paths: job.shift_roles.role_name, job.shifts.outlets.name */}
                    <p style={s.jobTitle}>{job.shift_roles?.role_name || "—"} — {job.shifts?.outlets?.name || "—"}</p>
                    <p style={s.jobMeta}>
                      {fmtDate(job.shifts?.shift_date)} · {fmtTime(job.shifts?.start_time)} – {fmtTime(job.shifts?.end_time)}
                    </p>
                  </div>

...
import {
  ...
  Award, TrendingUp, Scale, Loader2, Settings2, Zap, Pencil, X, Save, RotateCcw, Lock,
} from "lucide-react";
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: "rotate(-90deg)", filter: "drop-shadow(0 2px 8px rgba(0,0,0,0.06))" }}>
      ...
      {arcs.map((arc, i) => (
        <circle key={i} cx={cx} cy={cy} r={r} fill="none"
          ...
          strokeDasharray={`${Math.max(0, (arc.pct - gap) * circumference)} ${circumference}`}
              ...
              <div style={{ display: "flex", gap: "16px", marginBottom: "28px" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    <input readOnly value={(() => { const [h, m] = settings.open_time.split(":"); const hr = +h; return `${hr % 12 || 12}:${m} ${hr < 12 ? "AM" : "PM"}`; })()} style={{ ...disabledInput, paddingLeft: "34px" }} />
                  </div>
                </div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ position: "relative" }}>
                    <Clock size={14} color="#94A3B8" style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)" }} />
                    <input readOnly value={(() => { const [h, m] = settings.close_time.split(":"); const hr = +h; return `${hr % 12 || 12}:${m} ${hr < 12 ? "AM" : "PM"}`; })()} style={{ ...disabledInput, paddingLeft: "34px" }} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "24px" }}>
                {[
                  ...
                  <div key={label}>
                    ...
                    <input readOnly value={value} style={disabledInput} />
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
              ...
              {!editingAlloc ? (
                ...
                <div style={{ display: "flex", gap: "6px" }}>
                  <button onClick={cancelAlloc} style={btnCancelSm}><X size={12} /> Cancel</button>
                  <button onClick={saveAllocation} disabled={saving || !allocValid} style={{ ...btnSaveSm, opacity: (saving || !allocValid) ? 0.6 : 1 }}>

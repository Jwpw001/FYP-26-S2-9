    ...
    <CoordinatorLayout title="Krewby Requests">
      ...
      : filtered.length === 0 ? (
        ...
        <div key={req.request_id} style={s.card} onClick={() => navigate(`/krewby-coordinator/requests/${req.request_id}`)}>
          <div style={s.cardTop}>
            <div>
              <p style={s.reqTitle}>{req.role_name} — {req.outlet?.name || "Outlet"}</p>
              <p style={s.reqMeta}>{fmtDate(req.shift_date)} · {req.start_time?.slice(0,5)} – {req.end_time?.slice(0,5)}</p>
              <p style={s.reqSkill}>Requires: {req.skill?.name || req.skill_id}</p>
            </div>
            <div style={s.right}>

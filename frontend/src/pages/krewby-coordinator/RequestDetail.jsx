...
export default function CoordinatorRequestDetail() {
  ...
  async function handleApprove(workerId, isOverride) {
    ...
    try {
      await api.post(`/api/krewby/requests/${id}/assign`, { worker_id: workerId, override_note: isOverride ? overrideNote : null });
    ...
    <CoordinatorLayout title="Request Detail">
      ...
      <div style={s.card}>
        ...
        <div style={s.infoGrid}>
          <div style={s.infoItem}><span style={s.infoLabel}>Outlet</span><span style={s.infoVal}>{request.outlet?.name}</span></div>
          ...
          <div style={s.infoItem}><span style={s.infoLabel}>Time</span><span style={s.infoVal}>{request.start_time?.slice(0,5)} – {request.end_time?.slice(0,5)}</span></div>
          <div style={s.infoItem}><span style={s.infoLabel}>Skill</span><span style={s.infoVal}>{request.skill?.name || "—"}</span></div>
      ...
      {request.status === "pending_review" && (
        <div style={s.matchSection}>
          ...
          {matches.length === 0 ? (
            ...
            <div key={match.worker_id} style={s.matchCard}>
              <div style={s.matchTop}>
                ...
                <div style={s.matchInfo}>
                  ...
                  <div style={s.scoreRow}>
                    <span style={s.scoreItem}>⭐ {match.rating?.toFixed(1) || "5.0"}</span>
                ...
                <div style={s.matchActions}>
                  {i === 0 ? (
                    <button style={s.approveBtn} onClick={() => handleApprove(match.worker_id, false)} disabled={processing === match.worker_id}>
                      {processing === match.worker_id ? "…" : "Approve"}
                    </button>
                  ) : (
                    showOverride === match.worker_id ? (
                      <div style={s.overrideForm}>
                        <textarea style={s.overrideInput} placeholder="Override reason..." value={overrideNote} onChange={e => setOverrideNote(e.target.value)} />
                        <button style={s.approveBtn} onClick={() => handleApprove(match.worker_id, true)} disabled={processing === match.worker_id}>Confirm Override</button>
                        <button style={s.cancelBtn} onClick={() => setShowOverride(null)}>Cancel</button>
                      </div>
                    ) : (
                      <button style={s.overrideBtn} onClick={() => setShowOverride(match.worker_id)}>Override & Select</button>
                    )
                  )}
                </div>
              </div>
...
const s = {
  ...
  scoreRow:{ display:"flex", gap:"12px", marginTop:"6px" },

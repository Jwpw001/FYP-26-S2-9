...
export default function RegularMyShifts() {
  const user = getUser();
  ...
  const myAssignments = shifts.flatMap(shift =>
    (shift.shift_roles || []).flatMap(role =>
      (role.shift_assignments || [])
        .filter(a => a.staff_id === user?.staff_id)
        ...
        <div key={a.assignment_id} style={s.card}>
          <div style={s.cardTop}>
            <div>
              ...
              <p style={s.shiftMeta}>{fmtDate(a.shift.shift_date)} · {a.shift.start_time?.slice(0,5)} – {a.shift.end_time?.slice(0,5)}</p>

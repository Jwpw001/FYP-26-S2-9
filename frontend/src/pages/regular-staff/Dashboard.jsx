...
function fmtDay(d) {
  if (!d) return "";
  return new Date(d + "T00:00:00").toLocaleDateString("en-SG", { weekday:"short" }).toUpperCase();
}
...
export default function StaffDashboard() {
  ...
  const quickActions = [
    { label:"My Shifts & Reports", icon: FileText,    link:"/regular-staff/shifts" },
  ...
  return (
    <StaffLayout title="Dashboard">
      <div style={{ animation:"pageIn 0.4s ease both" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(180px,1fr))", gap:"16px", marginBottom:"28px" }}>
            {statCards.map((card, i) => (
              <div key={card.label} className="staff-stat-card" onClick={() => navigate(card.link)}
                style={{ background:"#FFF", border:"1px solid #E2E8F0", borderRadius:"16px", padding:"22px",
                  cursor:"pointer", boxShadow:"0 1px 4px rgba(0,0,0,0.05)", transition:"transform 0.18s, box-shadow 0.18s",
                  animation:`fadeSlideUp 0.35s ease ${i*0.07}s both` }}>
                <div style={{ width:"44px", height:"44px", borderRadius:"10px", background:card.bg, color:card.color,
        ...
        {/* Today's Shift */}
        <div style={{ ...s.section,
          background: todayShift ? "linear-gradient(135deg,#EFF6FF 0%,#F0FDF4 100%)" : "#FAFAFA",
          border: todayShift ? "1.5px solid #BFDBFE" : "1px solid #E2E8F0" }}>
          ...
          {loading ? (
            <div style={{ display:"flex", gap:"16px", alignItems:"center" }}>
              ...
              <Shimmer w="130px" h="40px" r="10px" />
            </div>
          ) : todayShift === false ? (
            <div style={{ textAlign:"center", padding:"24px 0" }}>
              <SmilePlus size={32} color="#64748B" style={{ margin:"0 auto 10px" }} />
              <p style={{ fontSize:"15px", fontWeight:"700", color:"#1E293B" }}>No shift today — take a well-earned rest!</p>
              <p style={{ fontSize:"13px", color:"#64748B", marginTop:"4px" }}>Check back tomorrow.</p>
            </div>
          ) : (
            <div style={{ display:"flex", alignItems:"center", gap:"16px", flexWrap:"wrap" }}>
              {/* Date block */}
              <div style={{ minWidth:"56px", textAlign:"center", background:"#2563EB", borderRadius:"12px", padding:"10px 6px", flexShrink:0 }}>
          ...
          {loading ? (
            ...
            upcomingShifts.map((a, i) => (
              <div key={a.assignment_id}
                style={{ display:"flex", alignItems:"center", gap:"16px", padding:"14px 0",
                  borderBottom:"1px solid #F1F5F9", animation:`fadeSlideUp 0.3s ease ${i*0.06}s both` }}>
            ...
            {quickActions.map(a => (
              <button key={a.label} className="staff-action-btn" onClick={() => navigate(a.link)}
                style={{ display:"flex", alignItems:"center", gap:"8px", padding:"10px 18px",
                  ...
                  fontSize:"13px", fontWeight:"600", color:"#1E293B", cursor:"pointer", transition:"all 0.15s" }}>
...
const s = {
  section: { background:"#FFF", border:"1px solid #E2E8F0", borderRadius:"16px", padding:"22px", marginBottom:"20px", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" },

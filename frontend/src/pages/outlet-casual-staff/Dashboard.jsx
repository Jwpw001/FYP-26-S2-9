...
export default function CasualDashboard() {
  ...
  const user = getUser();
  const [stats, setStats] = useState({ upcomingShifts:0 });
    ...
    <CasualLayout title="Dashboard">
      {loading ? <div style={s.loading}>Loading…</div> : (
        <>
          ...
          <div style={s.grid}>
            <div style={s.card} onClick={() => navigate("/outlet-casual-staff/shifts")}>
          ...
          <div style={s.actions}>
            {[
              { label:"My Shifts", icon:"📅", link:"/outlet-casual-staff/shifts" },
              { label:"My Availability", icon:"📆", link:"/outlet-casual-staff/availability" },
            ].map(a => (
...
const s = {
  ...
  nextShift:{ background:"#FFFFFF", border:"1px solid #E5E2DC", borderRadius:"14px", padding:"20px", marginBottom:"20px" },

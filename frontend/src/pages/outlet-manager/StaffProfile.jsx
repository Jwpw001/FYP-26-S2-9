  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setSelectedSkills((profileRes.data?.user_skill_tags || []).map(t => t.skill_id));
      ...
      <div style={s.card}>
        <div style={s.profileTop}>
          ...
          <div>
            ...
            <span style={{ ...s.typeBadge, background: profile.staff_type === "regular" ? "#DBEAFE" : "#F5F3FF", color: profile.staff_type === "regular" ? "#1E40AF" : "#6D28D9" }}>
              {profile.staff_type === "regular" ? "Regular Staff" : "Outlet Casual Staff"}
            </span>
          </div>
          <button style={{ ...s.activeBtn, background: profile.is_active ? "#FEF2F2" : "#DCFCE7", color: profile.is_active ? "#991B1B" : "#166534" }}
        ...
        <div style={s.grid}>
          <div style={s.field}>
            ...
            <input style={s.input} value={profile.users?.full_name || ""} onChange={e => setProfile(prev => ({ ...prev, users: { ...prev.users, full_name: e.target.value } }))} />
          </div>
          <div style={s.field}>
            <label style={s.label}>Email</label>
            <input style={s.input} value={profile.users?.email || ""} onChange={e => setProfile(prev => ({ ...prev, users: { ...prev.users, email: e.target.value } }))} />
          </div>
          <div style={s.field}>
            <label style={s.label}>Staff Type</label>
            <select style={s.input} value={profile.staff_type} onChange={e => setProfile(prev => ({ ...prev, staff_type: e.target.value }))}>
          ...
          <div style={s.field}>
            ...
            <input style={s.input} type="date" value={profile.hired_at?.split("T")[0] || ""} onChange={e => setProfile(prev => ({ ...prev, hired_at: e.target.value }))} />
          </div>
        </div>

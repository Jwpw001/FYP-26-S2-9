...
export default function ManagerDashboard() {
  ...
  const [stats, setStats] = useState({
    upcomingShifts: 0, pendingLeave: 0,
    pendingSwaps: 0, totalStaff: 0,
  });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const [{ data: shifts }, { count: staffCount },
               { count: leaveCount }, { count: swapCount }] = await Promise.all([
  ...
  const cards = [
    { label: "Upcoming Shifts", value: stats.upcomingShifts, sub: "Next 7 days",
      icon: "📅", color: "#2563EB", bg: "#EFF6FF", link: "/outlet-manager/shifts" },
    { label: "Total Staff", value: stats.totalStaff, sub: "Active members",
      icon: "👥", color: "#059669", bg: "#ECFDF5", link: "/outlet-manager/staff" },
    { label: "Pending Leave", value: stats.pendingLeave, sub: "Awaiting approval",
      icon: "📋", color: "#D97706", bg: "#FFFBEB", link: "/outlet-manager/availability" },
    { label: "Swap Requests", value: stats.pendingSwaps, sub: "Needs review",
      icon: "🔄", color: "#7C3AED", bg: "#F5F3FF", link: "/outlet-manager/shifts" },
  ];
        ...
        <>
          ...
          <div style={s.section}>
            <div style={s.sectionHeader}>
              ...
              <button style={s.viewAll} onClick={() => navigate("/outlet-manager/shifts")}>
                View all →
              </button>
            </div>
            {recentShifts.length === 0 ? (
              <div style={s.empty}>No shifts in the next 7 days.</div>
            ) : (
              <div style={s.table}>
                ...
                {recentShifts.map((shift) => (
                  <div key={shift.shift_id} style={s.tableRow}
                    ...
                    <span>
                      <span style={{ ...s.badge, ...badgeStyle(shift.status) }}>
                        {shift.status}
                      </span>
                    </span>
                  </div>
...
const s = {
  ...
  section: { background:"#FFFFFF", border:"1px solid #E5E2DC", borderRadius:"14px",
  ...
  table: { width:"100%" },
  tableHead: { display:"grid", gridTemplateColumns:"1fr 2fr 1fr 1fr",
    padding:"8px 12px", background:"#F7F6F3", borderRadius:"8px",
    fontSize:"12px", fontWeight:"600", color:"#7A7870", marginBottom:"4px", gap:"8px" },
  tableRow: { display:"grid", gridTemplateColumns:"1fr 2fr 1fr 1fr",
    padding:"10px 12px", borderRadius:"8px", cursor:"pointer",
    fontSize:"13px", gap:"8px", alignItems:"center",
    borderBottom:"1px solid #F0EDE8", color:"#1C1B18" },

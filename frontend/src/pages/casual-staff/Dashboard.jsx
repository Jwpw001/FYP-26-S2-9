      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px", animation: "fadeUp .4s ease both" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#64748B", margin: 0 }}>Casual worker{branchName ? ` · ${branchName}` : ""}</p>
          </div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <span style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", fontWeight: "700", color: isActive ? "#16A34A" : "#DC2626", background: isActive ? "#DCFCE7" : "#FEE2E2", border: `1px solid ${isActive ? "#BBF7D0" : "#FECACA"}`, padding: "6px 12px", borderRadius: "100px" }}>
              <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: isActive ? "#16A34A" : "#DC2626" }} />
              {isActive ? "Active" : "Inactive"}
            </span>
            {!loading && upcomingShifts.length > 0 && (
              <span style={{ fontSize: "12px", fontWeight: "700", color: "#2563EB", background: "#EFF6FF", border: "1px solid #BFDBFE", padding: "6px 12px", borderRadius: "100px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: "18px" }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: "16px", padding: "22px 24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px", flexWrap: "wrap", gap: "8px" }}>
              <h3 style={{ fontSize: "15px", fontWeight: "700", color: "#1E293B", margin: 0 }}>{monthLabel}</h3>
              <div style={{ display: "flex", gap: "10px", fontSize: "11px", color: "#64748B", alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: "6px", marginBottom: "6px" }}>
              {DAY_LABELS.map(d => <span key={d} style={{ fontSize: "10px", fontWeight: "700", color: "#94A3B8", textAlign: "center" }}>{d}</span>)}
            </div>
            {/* Calendar cells */}
            {loading ? (
                ...
                {cells.map((c, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => setSelectedDate(c.date)}
                      style={{ minHeight: "42px", borderRadius: "9px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", padding: "5px 3px 4px", fontSize: "12px", fontWeight: c.isToday ? 800 : 600, cursor: "pointer", position: "relative", color, background: bg, border: `1.5px solid ${border}` }}>
                      <span>{c.date}</span>
                      {c.hasShift && (
                        <div style={{ display: "flex", gap: "2px", marginTop: "3px", justifyContent: "center", flexWrap: "wrap" }}>
                          ...
                          {c.dayShifts.length > 2 && (
                            <span style={{ fontSize: "8px", fontWeight: "700", color: "#2563EB", lineHeight: "5px" }}>+{c.dayShifts.length - 2}</span>
                          )}
                        </div>
                      )}
                      {c.hasUnacked && (
            ...
            <div style={{ marginTop: "14px", paddingTop: "14px", borderTop: "1px solid #F1F5F9" }}>
              <div style={{ fontSize: "12px", fontWeight: "700", color: "#94A3B8", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.04em" }}>{selLabel}</div>
              {selShifts.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: "5px", marginBottom: "8px" }}>
                  {selShifts.map((sh, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 10px", borderRadius: "8px", background: "#EFF6FF", border: "1px solid #BFDBFE" }}>
                      ...
                      <span style={{ fontSize: "12.5px", fontWeight: "700", color: "#1E293B", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sh.title || "Shift"}</span>
                      <span style={{ fontSize: "11.5px", color: "#3B82F6", fontWeight: "600", flexShrink: 0 }}>{sh.start_time?.slice(0,5)} – {sh.end_time?.slice(0,5)}</span>
                    </div>
                  ))}
                </div>
              )}
              <div style={{ fontSize: "12.5px", color: "#475569" }}>{selInfo}</div>
            </div>
          </div>

          {/* This Week's Hours */}
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: "16px", padding: "22px 24px" }}>
            <h3 style={{ fontSize: "15px", fontWeight: "700", color: "#1E293B", margin: "0 0 14px" }}>This Week's Hours</h3>
            {loading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "7px" }}>
                {weekSlots.map((w, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 12px", borderRadius: "9px", background: w.set ? "#EFF6FF" : "#FAFAFA", border: `1px solid ${w.set ? "#BFDBFE" : "#F1F5F9"}` }}>
                    <span style={{ width: "30px", fontSize: "12px", fontWeight: "700", color: w.set ? "#1E40AF" : "#94A3B8" }}>{w.day}</span>
                    {w.set ? (
                      <span style={{ fontSize: "12.5px", fontWeight: "500", color: "#1E293B" }}>{w.hours}</span>
                    ) : (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
          ...
          {/* Upcoming Gigs */}
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: "16px", padding: "22px 24px" }}>
            <h3 style={{ fontSize: "15px", fontWeight: "700", color: "#1E293B", margin: "0 0 14px" }}>Upcoming Gigs</h3>
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                ...
                <p style={{ fontSize: "13px", color: "#94A3B8", margin: 0 }}>No upcoming gigs yet.</p>
                  ...
                  return (
                    <div key={a.assignment_id} style={{ display: "flex", alignItems: "center", gap: "12px", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: "10px", padding: "11px 14px" }}>
                      <div style={{ width: "42px", height: "42px", borderRadius: "9px", background: "#DCFCE7", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <span style={{ fontSize: "10px", fontWeight: "800", color: "#16A34A", lineHeight: 1 }}>{d.toLocaleDateString("en-US", { month: "short" }).toUpperCase()}</span>
                        <span style={{ fontSize: "15px", fontWeight: "800", color: "#166534", lineHeight: 1 }}>{d.getUTCDate()}</span>
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: "13px", fontWeight: "600", color: "#1E293B", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sh.title || "Shift"}</p>
                        <p style={{ fontSize: "11.5px", color: "#94A3B8", margin: "1px 0 0" }}>{sh.start_time?.slice(0,5)} – {sh.end_time?.slice(0,5)}</p>
                      </div>
                      {!a.acknowledged && (
                        <span style={{ fontSize: "9px", fontWeight: "700", color: "#D97706", background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: "5px", padding: "2px 6px", flexShrink: 0 }}>NEW</span>
                      )}
                    </div>
                  );
                })}
          ...
          {/* Branch card */}
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: "16px", padding: "22px 24px" }}>
            <h3 style={{ fontSize: "15px", fontWeight: "700", color: "#1E293B", margin: "0 0 14px" }}>Branch</h3>
            {loading ? (
              ...
              <>
                <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "16px" }}>
                  ...
                  <div>
                    <p style={{ fontSize: "14px", fontWeight: "700", color: "#1E293B", margin: 0 }}>{branchName || "No branch assigned"}</p>
                    {branchAddr && <p style={{ fontSize: "12px", color: "#94A3B8", margin: "2px 0 0" }}>{branchAddr}</p>}
                  </div>
                </div>
                <button onClick={() => navigate("/casual-staff/availability")}
                  style={{ width: "100%", background: "#16A34A", color: "#fff", border: "none", borderRadius: "9px", padding: "11px", fontSize: "13px", fontWeight: "700", cursor: "pointer" }}>
                  Update Availability
                </button>
              </>
            ) : (
              <p style={{ fontSize: "13px", color: "#94A3B8", textAlign: "center", padding: "20px 0", margin: 0 }}>No branch assigned yet.</p>
            )}
          </div>
        </div>

...
export default function AdminReportHistory() {
  ...
  return (
    <AdminLayout title="Report History">
      <div style={{ animation: "pageIn 0.4s ease both" }}>

        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
          <button onClick={() => goTo("/system-admin/reports")}
          ...
          <div>
            <h2 style={{ fontSize: "22px", fontWeight: "800", color: "#0F172A" }}>Report History</h2>
            <p style={{ fontSize: "13px", color: "#64748B", marginTop: "2px" }}>All reports exported across all roles</p>
          </div>
        </div>

        <div style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: "16px", padding: "22px 24px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          {loading ? (
            <p style={{ color: "#94A3B8", fontSize: "13px", textAlign: "center", padding: "32px 0" }}>Loading...</p>
          ) : rows.length === 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "560px" }}>
                <thead>
                  <tr style={{ borderBottom: "2px solid #F1F5F9" }}>
                    {["Title", "Format", "Period", "Exported By", "Downloaded"].map(h => (
                ...
                <tbody>
                  {rows.map(r => (
                    <tr key={r.report_id} style={{ borderBottom: "1px solid #F8FAFC" }}>
                      ...
                      <td style={{ padding: "12px", textAlign: "center", color: "#94A3B8", fontSize: "12px", whiteSpace: "nowrap" }}>

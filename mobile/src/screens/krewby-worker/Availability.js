...
export default function WorkerAvailability() {
  ...
  const load = useCallback(async () => {
    ...
    const id = kw?.[0]?.krewby_worker_id;
    setKwId(id);

    const fresh = initDays();
    if (id) {
      const { data } = await supabase.from("worker_availability")
        .select("*").eq("krewby_worker_id", id).eq("week_start", weekStartStr);
      (data || []).forEach(row => {
  ...
  function toggle(day) { setAvail(p => ({ ...p, [day]: { ...p[day], available: !p[day].available } })); }
  function setTime(day, field, val) { setAvail(p => ({ ...p, [day]: { ...p[day], [field]: val } })); }

  async function save() {
    if (!kwId) return;
    setSaving(true);
    try {
      await supabase.from("worker_availability").delete().eq("krewby_worker_id", kwId).eq("week_start", weekStartStr);
      const rows = DAYS.filter(d => avail[d].available).map(d => ({
        krewby_worker_id: kwId, week_start: weekStartStr, day_of_week: d,
        start_time: avail[d].start_time, end_time: avail[d].end_time,
      }));
      if (rows.length > 0) await supabase.from("worker_availability").insert(rows);
          ...
          {DAYS.map(day => (
            <View key={day} style={[s.dayCard, avail[day].available && s.dayCardActive]}>
              <View style={s.dayTop}>
                ...
                <Switch value={avail[day].available} onValueChange={() => toggle(day)}
                  trackColor={{ false: "#E2E8F0", true: "#2563EB" }} thumbColor="#fff" />
              </View>
              {avail[day].available && (
                <View style={s.timeRow}>
                  <View style={s.timeField}>
                    <Text style={s.timeLabel}>From</Text>
                    <TextInput style={s.timeInput} value={avail[day].start_time} onChangeText={v => setTime(day,"start_time",v)} placeholder="09:00" placeholderTextColor="#94A3B8" />
                  </View>
                  <Text style={s.timeDash}>–</Text>
                  <View style={s.timeField}>
                    <Text style={s.timeLabel}>Until</Text>
                    <TextInput style={s.timeInput} value={avail[day].end_time} onChangeText={v => setTime(day,"end_time",v)} placeholder="18:00" placeholderTextColor="#94A3B8" />
                  </View>
                </View>
              )}
            </View>

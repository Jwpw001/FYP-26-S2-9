...
// Runs a single AI-assistant tool call and returns its outcome, without touching the HTTP
// response — shared by the manual /execute endpoint (one confirm-card click) and the chat
// controller's bulk auto-execute loop (many sequential calls within one streamed turn).
async function runTool(tool_name, args, userId, role) {
  if (role !== "manager") {
    return { success: false, statusCode: 403, message: "Action capabilities are for managers only." };
  }
  ...
  if (!branchId) {
    return { success: false, statusCode: 403, message: "No branch found for your account." };
  }
  ...
  if (tool_name === "approve_leave") {
    ...
    if (!leave || leave.staff?.branch_id !== branchId) {
      return { success: false, statusCode: 403, message: "Leave request not found or access denied." };
    }
    if (leave.status !== "pending") {
      return { success: false, statusCode: 400, message: "This leave request is no longer pending." };
    }
    ...
    return { success: true, statusCode: 200, message: `Leave approved for ${staff_name}.` };
  }
  ...
  if (tool_name === "reject_leave") {
    ...
    return { success: true, statusCode: 200, message: `Leave rejected for ${staff_name}${reason ? ` (reason: ${reason})` : ""}.` };
  }
  ...
  if (tool_name === "create_draft_shift") {
    ...
    if (!title || !shift_date || !start_time || !end_time) {
      return { success: false, statusCode: 400, message: "Missing required shift fields." };
    }
    ...
    return {
      success: true, statusCode: 200, shift_id: shift.shift_id,
  ...
  if (tool_name === "set_staff_active") {
    ...
    if (!staffMember) {
      return { success: false, statusCode: 403, message: "Staff member not found in your branch." };
    }
  ...
  if (tool_name === "add_task_to_shift") {
    ...
    if (!shift || shift.branch_id !== branchId) {
      return { success: false, statusCode: 403, message: "Shift not found or access denied." };
    }
    ...
    return {
      success: true, statusCode: 200, task_id: lastTaskId,
  ...
  if (tool_name === "publish_shift") {
    ...
    if (shift.status === "published") {
      return { success: false, statusCode: 400, message: `Shift "${shift_title}" is already published.` };
    }
    ...
    return { success: true, statusCode: 200, message: `Shift "${shift_title}" has been published. Staff can now see and acknowledge their assignments.` };
  }
  ...
  if (tool_name === "assign_staff_to_task") {
    ...
    if (!task || task.shifts?.branch_id !== branchId) {
      return { success: false, statusCode: 403, message: "Task not found or access denied." };
    }
    ...
    if (existing) {
      return { success: false, statusCode: 400, message: `${staff_name} is already assigned to this task.` };
    }
    ...
    return { success: true, statusCode: 200, message: `${staff_name} has been assigned to "${task_name}" in "${shift_title}".` };
  }
...
async function execute(req, res) {
  try {
    ...
    const result = await runTool(tool_name, args, userId, role);
    return res.status(result.statusCode || 200).json({ success: result.success, message: result.message });
  } catch (err) {
...
module.exports = { execute, runTool, ALLOWED_TOOLS };
